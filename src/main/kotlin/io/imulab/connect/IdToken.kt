...
interface IdTokenStrategy {
    ...
    /**
     * Validate and decode an id token. The token must be generated by this strategy previously and have not
     * expired. If the client requires `id_token` to be encrypted, and the encryption uses asymmetric keys, the
     * client must first decrypt the token and re-encrypt it with a key that enables this service to decrypt it
     * before sending, otherwise the decoding process will fail.
     */
    fun decodeToken(token: String, request: Request): JwtClaims
}
...
class JwxIdTokenStrategy(
    ...
    override fun decodeToken(token: String, request: Request): JwtClaims {
        var wip = token

        if (request.client.requireIdTokenEncryption()) {
            wip = doDecrypt(token, request.client)
        }

        if (request.client.idTokenSignedResponseAlgorithm == SigningAlgorithm.NONE)
            return JwtClaims.parse(wip)

        return doVerify(wip, request.client)
    }
    ...
    private fun doVerify(raw: String, client: Client): JwtClaims {
        val key = if (client.idTokenEncryptedResponseAlgorithm.symmetric) {
            AesKey(client.resolvePlainTextSecret().toByteArray(StandardCharsets.UTF_8))
        } else {
            val kid = JwtConsumerBuilder()
                .setSkipAllValidators()
                .setDisableRequireSignature()
                .setSkipSignatureVerification()
                .build()
                .process(raw)
                .joseObjects.firstOrNull { it.keyIdHeaderValue != null && it.keyIdHeaderValue.isNotEmpty() }
                ?.keyIdHeaderValue ?: throw Errors.serverError("cannot locate verification key for id token.")
            jwks.findJsonWebKey(kid, null, Use.SIGNATURE, null).resolvePublicKey()
        }
    ...
    private fun doEncrypt(raw: String, client: Client, clientJwks: JsonWebKeySet): String {
        val jwk = clientJwks.selectKeyForEncryption(client.idTokenEncryptedResponseAlgorithm)
        ...
        return JsonWebEncryption().also { jwe ->
            ...
            jwe.keyIdHeaderValue = jwk.keyId
        }.compactSerialization
    }

    private fun doDecrypt(raw: String, client: Client): String {
        val firstPass = JsonWebEncryption().apply { compactSerialization = raw }

        val decryptionKey = if (client.idTokenEncryptedResponseAlgorithm.symmetric) {
            AesKey(client.resolvePlainTextSecret().toByteArray(StandardCharsets.UTF_8))
        } else {
            jwks.findJsonWebKey(firstPass.keyIdHeaderValue, null, Use.ENCRYPTION, null).resolvePrivateKey()
        }

        val encryptionAlgorithm = if (client.idTokenEncryptedResponseAlgorithm.symmetric) {
            client.idTokenEncryptedResponseAlgorithm.value
        } else {
            firstPass.keyManagementModeAlgorithm.algorithmIdentifier
        }

        val encryptionEncoding = if (client.idTokenEncryptedResponseAlgorithm.symmetric) {
            client.idTokenEncryptedResponseEncoding.value
        } else {
            firstPass.contentEncryptionAlgorithm.algorithmIdentifier
        }

        return JsonWebEncryption().apply {
            setAlgorithmConstraints(
                AlgorithmConstraints(AlgorithmConstraints.ConstraintType.WHITELIST, encryptionAlgorithm))
            setContentEncryptionAlgorithmConstraints(
                AlgorithmConstraints(AlgorithmConstraints.ConstraintType.WHITELIST, encryptionEncoding))
            compactSerialization = raw
            key = decryptionKey
        }.plaintextString
    }
}